...
export const Video = ({
  ...
  const handleOrientation = useCallback(async () => {
    if (isFullscreen && isMobile) {
      try {
        // First unlock to prevent conflicts
        screen?.orientation?.unlock();
        // Short delay to ensure unlock completes
        ...
        if (error instanceof Error) {
          console.warn('Screen orientation API not supported:', error.message);
          toast.error("Screen orientation not supported");
        }
      }
    } else {
      try {
        ...
        if (error instanceof Error) {
          console.warn('Screen orientation unlock failed:', error.message);
        }
      }
    }
  ...
  useEffect(() => {
    ...
    const handleOrientationChange = () => {
      const isLandscapeNow = window.orientation === 90 || window.orientation === -90;
      setIsLandscape(isLandscapeNow);

      // Clear any pending lock attempts
      clearTimeout(lockTimeout);

      if (isFullscreen) {
        // Delay the lock attempt to avoid rapid switching
    ...
    return () => {
      ...
      // Make sure to unlock when cleaning up
      if (screen?.orientation) {
        screen.orientation.unlock();
      }
    };
  }, [isFullscreen, isMobile, handleOrientation]);
  ...
  const toggleFullscreen = useCallback(async () => {
    try {
      if (isFullscreen) {
        ...
        // Ensure we unlock orientation when exiting fullscreen
        ...
        if (isMobile) {
          // Add a small delay before trying to lock orientation
          setTimeout(() => {
            handleOrientation();
          }, 300);
        }
      ...
      console.warn('Fullscreen toggle failed:', error instanceof Error ? error.message : 'Unknown error');
    }
  }, [isFullscreen, isMobile, handleOrientation]);
            ...
            <AnimatePresence>
              {!isPlaying && (
                ...
                >
                  <motion.button
                    onClick={togglePlay}
                    className="flex items-center justify-center w-16 h-16 text-white bg-black/50 rounded-full"
                    ...
                    {settingsView !== "main" && (
                      ...
                      >
                        ...
                        <span className="text-sm font-medium">
                          ...
                          {settingsView === "subtitles" && "Subtitles & Audio"}
                        </span>
                      </motion.div>
                    )}
                          ...
                          <LayoutGroup>
                            {sleepOptions.map((option, index) => (
                              <motion.button
                                ...
                                onClick={() => {
                                  setSleepTimer(
                                    option.value === "off" ? null : option.value
                                  );
                                  navigateTo("main");
                                }}
                                className={cn(
