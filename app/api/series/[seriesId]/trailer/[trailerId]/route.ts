...
export async function GET(
  ...
  try {
    ...
    if (!seriesId) {
      return NextResponse.json("Series id not found", { status: 404 });
    }

    const trailer = await db.trailers.findMany({
      where: {
        id: trailerId,
        seriesId,
        isPublished: true,
...
export async function PATCH(
  ...
  try {
    ...
    const { languageCode, name, url, ...values } = await req.json();

    if (!user || user.role !== "ADMIN") {
      return new NextResponse("Unauthorized", { status: 401 });
    }

    const trailer = await db.trailers.update({
      where: {
        id: trailerId,
        seriesId,
      },
    ...
    if (values.videoUrl) {
      try {
        ...
        // Only create subtitle track if a valid subtitle URL is provided
        if (url && languageCode) {
